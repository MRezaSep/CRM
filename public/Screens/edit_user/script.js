...
const receivedString = urlParams.get("number");
let receviedNumber = receivedString;
// ===================================================================== >> Query <<
...
//
// -------------------------------------------------------- > Avatar <
const AvatarIcon = document.createElement("iconify-icon");
AvatarIcon.classList.add("AvatarIcon");
AvatarIcon.setAttribute("icon", "fluent:person-48-filled");
//
// -------------------------------------------------- AP >>
FormContianer.appendChild(AvatarIcon);
// -------------------------------------------------- AP <<
//
// -------------------------------------------------------- > Avatar <
//
...
const UserNamePlaceholder = "نام کامل کاربر را وارد کنید   ";
// ---------------------------------------- variables <<
//
// ----------------------------------------- CallBack >>
...
//
// -------------------------------------- appendChild >>
FormContianer.appendChild(UserNameFeild);
// -------------------------------------- appendChild <<
//
// ----------------------------------------------------- > User Name <
//
...
const AddressPlaceholder = "آدرس کاربر را وارد کنید   ";
// ---------------------------------------- variables <<
//
// ----------------------------------------- CallBack >>
...
//
// -------------------------------------- appendChild >>
FormContianer.appendChild(AddressFeild);
// -------------------------------------- appendChild <<
//
// ------------------------------------------------------- > Address <
//
...
const NationalCodePlaceholder = "کد ملی کاربر را وارد کنید   ";
// ---------------------------------------- variables <<
//
// ----------------------------------------- CallBack >>
...
//
// -------------------------------------- appendChild >>
FormFlexContainer.appendChild(NationalCodeFeild);
NationalCodeFeild.setAttribute("maxlength", "10");
// -------------------------------------- appendChild <<
//
// -------------------------------------------------- > NationalCode <
//
...
// ---------------------------------------------- variables >>
const JobLabel = "شغل";
const JobPlaceholder = "شغل کاربر را وارد کنید   ";
// ---------------------------------------------- variables <<
//
// ----------------------------------------------- CallBack >>
...
//
// -------------------------------------------- appendChild >>
FormFlexContainer.appendChild(JobFeild);
// -------------------------------------------- appendChild <<
//
// ----------------------------------------------------------- > Job <
//
...
function FeildValidator(Feild, maxlength) {
  //
  // Variables >>
  const FeildInput = Feild.childNodes[1];
  const FeildValue = Feild.childNodes[1].value;
  let IsValid = false;
  let MassageClass = "";
  let MassageText = "";
  let MassageIcon = "";
...
function Validator() {
  ...
  //
  // --------------- FullName V >
  let UserValid = FeildValidator(UserNameFeild, 5);
  // --------------- FullName V <
  //
  // ---------------- Address V >
  let AddressValid = FeildValidator(AddressFeild, 10);
  // ---------------- Address V <
  //
  // -------------------- Jpb V >
  let JobValid = FeildValidator(JobFeild, 3);
  // -------------------- Jpb V >
  //
  // ---------- National Code V >
  let NationalCodeValid = FeildValidator(NationalCodeFeild, 10);
  // ---------- National Code V >
  //
  // ----------------- Gender V >
...
function PostInformation() {
  // ------------------- ClassManagemnet >
  Layer4.classList.remove("remove");
  // ------------------- ClassManagemnet <
  //
  // ------------------------- Variables >
  const NameValue = UserNameFeild.childNodes[1].value;
  const Addressvalue = AddressFeild.childNodes[1].value;
  const NationalCodeValue = NationalCodeFeild.childNodes[1].value;
  const JobValue = JobFeild.childNodes[1].value;
  ...
  //
  // -------------------------- CallBack >
  let API = `https://hiwearedevelopers.monster/user/edit-${receviedNumber}`;
  //
  postData(API, Data)
    .then((response) => {
      //
      ...
      // 401 >>
        ...
        MassageClass = "error";
        MassageText = "لطفا اطلاعات را تغییر دهید سپس ثبت کنید";
        MassageIcon = "icon-park-solid:error";
        MassageCallBack(MassageClass, MassageText, MassageIcon);
      }
      // 401 <<
...
//
// ========================================================== >> Inputs Auto Fill <<
function InputsAutoFill() {
  //
  // Varibels >>
  const UserNameInput = UserNameFeild.childNodes[1];
  const AddressInput = AddressFeild.childNodes[1];
  const NationalCodeInput = NationalCodeFeild.childNodes[1];
  const JobInput = JobFeild.childNodes[1];
  // Varibels <<
  //
  // API >>
  let Api = `https://hiwearedevelopers.monster/user/single-${receviedNumber}`;
  // API <<
  //
  // GET >>
  fetchData(Api)
    .then((response) => {
      ...
      //
      // NationaCode >>
      NationalCodeInput.value = response.national_code;
      // NationaCode <<
      //
      // Gender >>
      ...
      //
      // Birth Date >>
      let BirthDayTitleTextWidget =
        BirthDayDatePicker.childNodes[1].childNodes[1];
      BirthDayTitleTextWidget.textContent = response.birth_date;
      BirthDay = response.birth_date;
      // Birth Date <<
      //
      // Marridge Date >>
      let MarridgeTitleTextWidget =
        MarridgeDayDatePicker.childNodes[1].childNodes[1];
      if (response.mrg_date !== "") {
        MarridgeTitleTextWidget.textContent = response.mrg_date;
      }
      MarridgeDate = response.mrg_date;
      // Marridge Date <<
    })
    .catch((error) => {
      console.log(error);
